...
export default function BankView() {
  const { questionBank, isLoading, fetchQuestionBank, addToQuestionBank, deleteFromQuestionBank } = useClassStore();
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    const success = await addToQuestionBank(newItem);
    if (success) {
      setIsAdding(false);
      setTitle('');
      setQuestionText('');
    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 h-full flex flex-col">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="font-mono text-xs font-bold text-on-surface-variant uppercase mt-2">Manage your interactive modules</p>
        </div>
        <button 
        ...
        >
          {isAdding ? 'Cancel' : <><Plus size={20} strokeWidth={3} /> Add Item</>}
        </button>
      </div>

      {isAdding && (
        <form onSubmit={handleAdd} className="bg-surface border-4 border-surface-dark p-6 shadow-[6px_6px_0px_#111827] mb-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pb-8">
        {isLoading && questionBank.length === 0 ? (
          <div className="col-span-full py-12 text-center font-mono font-bold uppercase animate-pulse">Loading bank...</div>
        ) : questionBank.length === 0 ? (
          <div className="col-span-full py-12 text-center font-mono font-bold uppercase border-4 border-dashed border-surface-dark/20 text-on-surface-variant">Question bank is empty.</div>
        ) : (
          questionBank.map((item) => (
            <div key={item.id} className="bg-surface border-4 border-surface-dark flex flex-col shadow-[6px_6px_0px_#111827] hover:-translate-y-1 transition-transform">
              <div className="p-4 border-b-4 border-surface-dark bg-surface-container-high flex justify-between items-start gap-4">
                ...
                <button onClick={() => deleteFromQuestionBank(item.id)} className="text-on-surface-variant hover:text-red-600 transition-colors">
                  <Trash2 size={20} />
                </button>
              </div>
              <div className="p-4 flex-1">
                <p className="text-sm font-medium text-on-surface-variant line-clamp-3 mb-4">{item.questionText}</p>
              </div>
