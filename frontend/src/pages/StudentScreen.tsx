...
export default function StudentScreen() {
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 w-full h-full z-0 flex items-center justify-center bg-white">
        {classState?.presentationUrl ? (
          ...
          <div className="w-full h-full flex flex-col items-center justify-center bg-white">
            <span className="font-semibold bg-blue-50 text-blue-700 px-4 py-1.5 rounded-full text-sm uppercase inline-block mb-6">Slide {classState.activeSlide}</span>
      ...
      {/* Floating In-place Fullscreen button for student */}
      <button
      ...
      {!classState.currentQuestion && !classState.isShowingLeaderboard && (
        <div className="absolute top-6 left-6 z-20 pointer-events-auto flex items-center gap-3 bg-white/40 hover:bg-white/90 backdrop-blur-md border border-slate-200/60 p-3 px-4 rounded-2xl shadow-xl opacity-60 hover:opacity-100 transition-all duration-300">
          ...
          <div>
            <h2 className="font-extrabold text-sm tracking-wide text-slate-800">{classState.className}</h2>
            <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Host: {classState.hostName}</p>
          </div>
        </div>
      )}
        ...
        <div className="absolute top-6 right-6 z-20 flex items-center gap-4 bg-white/40 hover:bg-white/90 backdrop-blur-md border border-slate-200/60 p-3 px-4 rounded-2xl shadow-xl pointer-events-auto opacity-60 hover:opacity-100 transition-all duration-300">
          ...
          <div className="flex items-center gap-4 bg-slate-50/50 rounded-xl p-2 px-3 border border-slate-200/50 shadow-sm relative">
            <div className="flex flex-col items-center relative">
              ...
              {scorePopup.visible && (
                <span className="absolute -top-6 left-1/2 -translate-x-1/2 text-blue-600 font-black text-xs animate-bounce whitespace-nowrap">+{scorePopup.points}</span>
              )}
            </div>
            ...
            <div className={`flex flex-col items-center transition-all duration-300 ${displayStreak >= 3 ? 'text-orange-500' : 'text-slate-400'}`}>
              ...
              <span className="font-black text-base flex items-center gap-1">
                <Flame size={14} className={displayStreak >= 3 ? 'animate-pulse text-orange-500' : ''}/> {displayStreak}
              </span>
            </div>
          </div>
          <button 
            onClick={handleLogout}
            className="text-slate-400 hover:text-red-500 hover:bg-red-50 p-2 rounded-xl transition-all border border-slate-200 flex items-center justify-center bg-white shadow-sm"
            title="Keluar Kelas"
          >
      ...
      {/* Whiteboard Toolbar Overlay */}
      {code && !classState.currentQuestion && !classState.isShowingLeaderboard && (
        <div className="absolute bottom-20 left-1/2 -translate-x-1/2 z-35 max-w-[90vw] pointer-events-auto">
      ...
      {classState?.isShowingLeaderboard && (
        <div className="absolute inset-0 z-40 bg-slate-900/60 backdrop-blur-sm p-4 md:p-8 overflow-y-auto flex items-center justify-center pointer-events-auto">
      ...
      {/* Quiz/Activity Overlay */}
      {classState.currentQuestion && (
        <div className="absolute inset-0 z-30 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4 md:p-8 overflow-y-auto pointer-events-auto">
          ...
          <div className="max-w-4xl w-full bg-white rounded-3xl shadow-2xl flex flex-col animate-in zoom-in-95 duration-300 overflow-hidden border border-slate-100 my-auto">
            ...
            <div className="flex-1 overflow-y-auto w-full p-4 md:p-8 bg-white relative min-h-[300px]">
                ...
                <>
                  {classState.currentQuestion.activityType === 'quiz' ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                      {classState.currentQuestion.options.map((opt: string, i: number) => {
